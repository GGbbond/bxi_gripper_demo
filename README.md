...
“启用软件限力”和力矩输入框用于设置最大夹持力矩，协议量程为 0.05–40 N·m。
请在电机下电时修改并点击“应用限力”。角度减小、夹爪继续夹紧时，后端在反馈力矩达到
设定值后冻结位置轨迹；超过设定值 5%时会缓慢向松开方向回退。力矩同时低于设定值的
90%后才恢复夹紧，避免在临界值附近高频启停。向松开方向的操作始终可用；力矩反馈超过
100 ms未更新时，后端也会暂停继续夹紧。

> 软件限力依赖 CAN 反馈和上位机控制周期，不能替代电机固件中的电流/力矩硬限制，瞬时力矩
> 仍可能略高于设定值。涉及人身或设备安全时，必须同时使用固件限流和机械保护。

如果上电后一直没有进入就绪状态，请检查 CAN 通道、电机 ID、接线、PCI 驱动和电机固件启动状态。

排查问题时，先确认连接日志中出现 `backend torque-limit-v4`，用于确认连接到新后端。
...
诊断文件中的 `torque_limit_*` 和 `filtered_abs_torque_Nm` 用于判断软件限力何时介入。
